...
module.exports = {
  ...
  addMahasiswa: async (req, res) => {
    try {
      ...
      if (error.code === 11000) {
        message = `NIM ${req.body.nim} Sudah Ada.`;
      }
  ...
  deleteMahasiswa: async (req, res) => {
    try {
      ...
      // Check if the mahasiswa document was found
      if (!mahasiswa) {
        req.flash("alertMessage", "No mahasiswa found with that ID");
        req.flash("alertStatus", "danger");
        res.redirect("/mahasiswa");
      ...
      await mahasiswa.deleteOne(); // or await Mahasiswa.deleteOne({ _id: id });
